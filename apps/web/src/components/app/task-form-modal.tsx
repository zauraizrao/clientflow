...
export function TaskFormModal({
  ...
  useEffect(() => {
    ...
    setAssigneeIds(
      current?.assignees.map(
        (assignee) =>
          assignee.organizationMemberId,
      ) ?? [],
    );
    setQuickSubtaskTitle("");
    setError(null);
  ...
  async function submit(
    ...
    if (isEdit && task.data) {
      ...
      try {
        ...
        if (
          values.projectColumnId !==
          task.data.projectColumnId
        ) {
  ...
  return (
    <ModalShell
      ...
      description={
        isEdit
          ? "Task details, workflow, subtasks and collaboration."
          : parentTaskId
