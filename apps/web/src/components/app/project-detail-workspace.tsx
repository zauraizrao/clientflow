...
export function ProjectDetailWorkspace({
  ...
  const tasks = useQuery({
    ...
    queryFn: () =>
      projectApi.listTasks(projectId, {
        ...
        scope: view === "board" ? "ROOT" : "ALL",
        page: view === "board" ? 1 : page,
        pageSize: view === "board" ? 100 : 20,
        sortBy: view === "board" ? "position" : "updatedAt",
        sortOrder: view === "board" ? "asc" : "desc",
      }),
  ...
  function openCreateTask(columnId?: string) {
    ...
    setNewTaskColumnId(
      columnId ?? activeColumns[0]?.id ?? null,
    );
    setTaskOpen(true);
  }
  ...
  async function toggleArchive() {
    if (!project.data || !canStructure) return;
    ...
    if (
      nextStatus === "ARCHIVED" &&
      !window.confirm(`Archive "${project.data.name}"?`)
    ) {
      return;
    }

    try {
      await projectApi.updateProject(projectId, { status: nextStatus });
      await refreshProject();
    } catch (error) {
      window.alert(
  ...
  if (project.isError || !project.data) {
    return (
      <div className="mx-auto max-w-[1480px] px-6 py-16">
        <div className="text-sm font-medium">Unable to load project</div>
  ...
  const availableTeam: ProjectTeamOptionDto[] = teamOptions.data ?? [];
      ...
      <section className="grid gap-5 lg:grid-cols-[160px_1fr] lg:items-start">
        ...
        <div>
          ...
          <div className="mt-6 grid gap-px overflow-hidden rounded-md border bg-border sm:grid-cols-2 xl:grid-cols-5">
            <Metric label="Client" value={data.client?.name ?? "Internal"} />
            <Metric label="Team" value={`${data.memberCount} members`} />
            <Metric label="Tasks" value={`${data.taskCount} total`} />
            <Metric
              label="Progress"
            ...
            <Metric
              label="Due"
              value={data.dueDate ? formatDate(data.dueDate) : "No due date"}
            />
          </div>
        </div>
      </section>
      ...
      <section className="mt-8 rounded-md border bg-card">
        <div className="flex flex-col gap-3 border-b p-3 xl:flex-row xl:items-center">
          ...
          <div className="font-mono text-[11px] text-muted-foreground">
            {pagination ? `${pagination.totalItems} results` : "—"}
          </div>
        </div>
        ...
        {tasks.isError ? (
          <div className="flex items-center justify-between gap-4 px-4 py-10">
            <div>
              <div className="text-sm font-medium">Unable to load tasks</div>
              <div className="mt-1 text-xs text-muted-foreground">
        ...
        {tasks.isSuccess && tasks.data.items.length === 0 ? (
          <div className="px-4 py-20 text-center">
            <div className="text-sm font-medium">No tasks match this view.</div>
            <div className="mt-1 text-xs text-muted-foreground">
              {canWriteTasks
