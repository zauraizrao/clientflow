...
export function InvoiceEditModal({
  ...
  async function save() {
    ...
    const parsed =
      updateInvoiceDraftSchema.safeParse({
        clientId: form.clientId,
        projectId:
          form.projectId || null,
        contactId:
    ...
    try {
      const saved =
        await invoiceApi.updateDraft(
          invoice.id,
          parsed.data as UpdateInvoiceDraftInput,
        );
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : "Unable to update invoice draft.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <ModalShell
      ...
      description="Draft financial fields remain editable until the invoice is finalized. Finalized invoices are immutable."
      width="max-w-6xl"
      onClose={onClose}
      footer={
        <>
          ...
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                ClientFlow recalculates authoritative totals when the draft is saved.
              </p>
            </div>
            <Button
              type="button"
